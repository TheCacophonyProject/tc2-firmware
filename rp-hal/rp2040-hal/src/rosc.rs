...
impl RingOscillator<Disabled> {
    ...
    /// Initializes the ROSC with a known frequency : frequency range is set, startup delay is calculated and set.
    /// See section 2.17.3. "Modifying the frequency" in the rp2040 datasheet for guidance on how to do this
    /// before initialising the ROSC.
    pub fn initialize_with_freq(self, freq_hz: u32) -> RingOscillator<Enabled> {
        self.device.ctrl.write(|w| w.enable().enable());
