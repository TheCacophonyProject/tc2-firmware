...
impl<D: SpiDevice, const DS: u8> Spi<Disabled, D, DS> {
    ...
    fn init_spi<F: Into<HertzU32>, B: Into<HertzU32>>(
        ...
        mode: &Mode,
        slave: bool,
    ) -> (Spi<Enabled, D, DS>, HertzU32) {
        self.device.reset_bring_down(resets);
        self.device.reset_bring_up(resets);

        let actual_rate = self.set_baudrate(peri_frequency, baudrate);
        ...
        (self.transition(Enabled { __private: () }), actual_rate)
    }
    ...
    pub fn init_slave(self, resets: &mut RESETS, mode: &Mode) -> (Spi<Enabled, D, DS>, HertzU32) {
