use crate::tests::helpers::{
    ...
    num_thermal_recordings_offloaded, offloaded_event_count, simulate_camera_with_config,
    startup_and_shutdown_recordings_made, test_start_and_end_time,
    test_start_and_end_time_with_min_duration,
};
...
#[test]
fn file_offload_errors_short_fixed_window() {
    // Cptv files to loop through for testing.
    let cptv_files = Some(vec![String::from("./test-fixtures/cat-trigger.cptv")]);
    let config = ConfigBuilder::new()
        .low_power_mode()
        .audio_disabled()
        .custom_recording_window("10:00", "11:00")
        .build();
    let (start_time, end_time) =
        test_start_and_end_time_with_min_duration(&config, Duration::hours(3));
    ...
    TEST_SIM_STATE.with(|s| {
        ...
        s.borrow_mut().offloads_fail_on_restart_iteration = Some(4);
    });
    ...
    TEST_SIM_STATE.with(|state| {
        ...
        assert_eq!(
            1,
            offloaded_event_count(&state.events_offloaded, LoggerEventKind::FileOffloadFailed)
        );
    });
}

#[test]
fn file_offload_errors_always_on() {
    // Cptv files to loop through for testing.
    let cptv_files = Some(vec![String::from("./test-fixtures/cat-trigger.cptv")]);
    let config = ConfigBuilder::new()
        .low_power_mode()
    ...
    TEST_SIM_STATE.with(|state| {
        ...
        assert_eq!(4, num_thermal_recordings_offloaded(&state.files_offloaded));
        assert_eq!(0, num_audio_recordings_offloaded(&state.files_offloaded));
        assert_eq!(
            0,
            num_audio_recordings_stored_in_flash(&state.flash_backing_storage)
