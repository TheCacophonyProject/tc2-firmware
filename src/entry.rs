...
use crate::clock_utils;
use crate::constants::{FIRMWARE_VERSION, ROSC_TARGET_CLOCK_FREQ_HZ};
...
pub fn real_main() {
    ...
    assert!(u8::try_from(system_clock_freq.to_Hz() / 1_000_000).is_ok());
    ...
    let mut fs = {
        let fs_cs = pins.gpio9.reconfigure();
        let fs_miso = pins.gpio8.reconfigure();
        let fs_mosi = pins.gpio11.reconfigure();
        let fs_clk = pins.gpio10.reconfigure();
        OnboardFlash::new(
            fs_cs,
            fs_mosi,
            fs_clk,
    ...
    let mut pi_spi = {
        let pi_ping = pins.gpio5.reconfigure();
        let pi_miso = pins.gpio15.reconfigure();
        let pi_mosi = pins.gpio12.reconfigure();
        let pi_cs = pins.gpio13.reconfigure();
        let pi_clk = pins.gpio14.reconfigure();
        ExtSpiTransfers::new(
            pi_mosi,
            pi_cs,
            pi_clk,
    ...
    if record_audio_now && let RecordingMode::Audio(recording_request_type) = recording_mode {
        ...
        let lepton_pins = LeptonPins {
            tx: pins.gpio23.reconfigure(),
            rx: pins.gpio20.reconfigure(),
            clk: pins.gpio22.reconfigure(),
            cs: pins.gpio21.reconfigure(),
            vsync: pins.gpio19.reconfigure(),
            sda: pins.gpio24.reconfigure(),
            scl: pins.gpio25.reconfigure(),
            power_down: pins.gpio28.reconfigure(),
            power_enable: pins.gpio18.reconfigure(),
            reset: pins.gpio29.reconfigure(),
            clk_disable: pins.gpio27.reconfigure(),
            master_clk: pins.gpio26.reconfigure(),
        };
        record_thermal(
            i2c,
            pi_spi,
